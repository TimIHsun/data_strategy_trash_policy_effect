...
```
```{r}
treatment.identifier <- "新北市"
controls.identifier <- setdiff(colnames(Trash_SCM[[1]]), treatment.identifier)
times.dep <- cbind("平均每人每日一般廢棄物產生量 (公斤)", c("2001-01","2010-12")) #dependant var

times.pred <- cbind( #predictor var
  "平均每戶消費支出" = c("2001-01","2010-12"),
  "失業率" = c("2001-01","2010-12"),
  "平均每戶人數"= c("2001-01","2010-12"),
  "平均每人可支配所得" = c("2001-01","2010-12"),
  "平均每人每日一般廢棄物產生量 (公斤)"=c("2001-01","2010-12")
)
```
...
```{r}
# res <- mscmt(Trash_SCM, treatment.identifier, controls.identifier, times.dep, times.pred, seed=1)
```
...
stargazer::stargazer(ols_ntp_chang_2)
```
...
ggplot(data = Changhua,aes(x= Changhua$統計期, y= Changhua$`平均每人每日一般廢棄物產生量 (公斤)`)) +
  ...
  geom_smooth()+
  geom_line(data = newTP ,aes(x= newTP$統計期, y= newTP$`平均每人每日一般廢棄物產生量 (公斤)`), color = "red")+
  geom_smooth(data = newTP, aes(x= newTP$統計期, y= newTP$`平均每人每日一般廢棄物產生量 (公斤)`)) -> Changhua_NTP
