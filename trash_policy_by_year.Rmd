...
write.csv(merged_trash_by_year, "trash_by_year.csv") #2016有重複(error: unbalanced data)
```
...
colnames(synntp_ntp) <- c("year","synthetic", "NTP")
```
...
```{r}
synthntp_ntp <- read_excel("synthntp_ntp.xlsx")
synthntp_ntp %>% mutate(
   treat = 統計區 == "NTP", 
   post  = 統計期>2010,
...
lm(synthntp_ntp_lmdata$每人每日 ~ treat+post+effect, data = synthntp_ntp_lmdata)-> syntheticlm
summary(syntheticlm)
```

```{r}
gaps[11:18] %>% mean() #mean effect
```
