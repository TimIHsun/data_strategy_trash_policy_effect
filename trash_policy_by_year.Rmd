...
---
```{r}
library(dplyr); library(stringr); library(ggplot2); library(plotly); library(lubridate); library(readr); library(tidyr); library(showtext); library(plm);library(readxl);library(MSCMT);library(Synth);library(reshape2)
...
lm(DIDNtpChang2$`平均每人每日一般廢棄物產生量 (公斤)`~ treat+post+effect, data = DIDNtpChang2)
...
```{r}
path.plot(synth.res = synth.out, dataprep.res = trashprep.out,
          Ylab = "平均每人每日一般廢棄物產生量 (公斤)") -> SCM_plot
```
